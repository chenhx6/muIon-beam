...
const note = [`# ${tag}`, '', '## 基于标签', '当前 main', '', '## 基础内容核验', '不适用于普通构建 tag', '', '## 本次任务', args.task || message, '', '## 本次调整', message, '', '## 优化内容', '自动提交、架构检查和远端分支校验已完成', '', '## 结果', '普通构建提交已通过项目架构检查', '', '## 主要限制', '该 tag 不代表正式研究结果，不包含结果级 Drive 归档和三端审计', '', '## 详细报告', args.report || '见对应 commit、测试输出和任务记录', '', '## Google Drive', args.drive_path || '不适用于普通构建 tag', ''].join('\n');
