...
const writeRun = (root, run) => { fs.mkdirSync(path.dirname(runFile(root, run.workflow_run_id)), { recursive: true }); jsonWrite(runFile(root, run.workflow_run_id), run); };
...
function event(root, run, event_type, stage, status, next_action = null, extra = {}) {
  const operation_id = `${run.workflow_run_id}:${stage}:${run.attempts || 0}`;
  const recorded = recordWorkflowEvent({ event_type, workflow_run_id: run.workflow_run_id, task_id: run.task_id, stage, status, operation_id, idempotency_key: operation_id, retry_count: run.attempts || 0, next_action, ...extra }, root);
  run.events.push(recorded);
  run.stage = stage; run.status = status; run.next_action = next_action; run.updated_at = nowIso();
  writeRun(root, run);
  return run;
}
function requireRun(root, id) { const run = readJson(runFile(root, id)); if (!run) throw new Error(`workflow run not found: ${id}`); return run; }
...
function advance(root, run) {
  ...
  if (run.stage === 'INTAKE') return taskCard(root, run.task_id) ? event(root, run, 'stage-completed', 'TASK_CARD_READY', 'READY', 'precheck') : event(root, run, 'stage-blocked', 'INTAKE', 'BLOCKED', 'create-task-card', { reason: 'task card not found' });
  if (run.stage === 'TASK_CARD_READY') run.stage = 'PRECHECK';
  ...
  if (run.stage === 'VALIDATING') {
    ...
    if (!resultText.includes('"status": "SUCCESS"')) return event(root, run, 'stage-blocked', 'VALIDATING', 'BLOCKED', 'diagnose-result', { reason: 'execution did not produce validated SUCCESS' });
  ...
  if (run.stage === 'REPORTING') {
    ...
    return event(root, run, 'stage-completed', 'ARCHIVING', 'READY', 'verify-archive');
  }
...
function main() {
  ...
  if (command === 'plan') run = createRun(root, args);
    ...
    if (['run', 'resume'].includes(command)) run = advance(root, run);
    else if (command === 'retry') { if (!['BLOCKED', 'FAILED_RETRYABLE'].includes(run.status)) throw new Error('retry requires a blocked or retryable workflow'); run.status = 'READY'; run.attempts += 1; writeRun(root, run); run = advance(root, run); }
      ...
      jsonWrite(requestPath, { schema_version: 1, task_id: run.task_id, workflow_run_id: run.workflow_run_id, close_requested: true, qa_passed: true, created_at: nowIso() });
      ...
      run = event(root, run, 'workflow-closed', 'CLOSED', 'CLOSE_REQUESTED', 'task-close execute', { output_refs: [run.close_request] });
    }
  }
