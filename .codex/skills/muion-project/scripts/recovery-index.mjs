...
export function buildRecoveryIndex(root, { driveRoot, git = args => runGit(root, args).stdout.trim(), now = nowIso() } = {}) {
  ...
  const cloudAudits = listJson(snapshotDir).map(file => readJson(file)).filter(item => ['snapshot-cloud-audit', 'drive-cloud-visibility-audit'].includes(item?.record_type)).sort((a, b) => (Date.parse(a?.recorded_at || a?.audited_at || '') || 0) - (Date.parse(b?.recorded_at || b?.audited_at || '') || 0));
  ...
  const driveCloudAudit = readJson(path.join(archiveRoot, 'project/recovery-index/cloud-visibility-audit.json'));
  if (driveCloudAudit?.record_type === 'drive-cloud-visibility-audit') cloudAudits.push(driveCloudAudit);
  cloudAudits.sort((a, b) => (Date.parse(a?.recorded_at || a?.audited_at || '') || 0) - (Date.parse(b?.recorded_at || b?.audited_at || '') || 0));
  const latestSnapshot = snapshots.at(-1) || null; const latestCloudAudit = cloudAudits.at(-1) || null;
  const mirrorManifest = readJson(path.join(archiveRoot, 'muIon-beam', 'drive-mirror-manifest.json'));
  const cloudAuditMatchesMirror = Boolean(
    mirrorManifest?.cloud_status === 'cloud-upload-verified' &&
    mirrorManifest?.verification === 'mapped-drive-count-size-sha256-verified' &&
    latestCloudAudit?.status === 'cloud-upload-verified' &&
    latestCloudAudit?.canonical_mirror_visible === true &&
    latestCloudAudit?.mirror_manifest_generated_at === mirrorManifest.generated_at &&
    latestCloudAudit?.mapped_drive?.file_count === mirrorManifest.file_count &&
    latestCloudAudit?.mapped_drive?.total_bytes === mirrorManifest.total_bytes
  );
  return {
    ...
    drive: { root: archiveRoot, canonical_mirror: mirrorManifest ? { path: path.join(archiveRoot, 'muIon-beam'), file_count: mirrorManifest.file_count, total_bytes: mirrorManifest.total_bytes, stale_count: mirrorManifest.stale_count || 0, stale_files: mirrorManifest.stale_files || [], verification: mirrorManifest.verification, cloud_status: mirrorManifest.cloud_status, manifest: 'muIon-beam/drive-mirror-manifest.json' } : null, latest_snapshot: latestSnapshot ? { snapshot_id: latestSnapshot.snapshot_id, commit: latestSnapshot.commit, path: latestSnapshot.drive_path, status: latestSnapshot.status, file_count: latestSnapshot.file_count, total_bytes: latestSnapshot.total_bytes, manifest_sha256: latestSnapshot.manifest_sha256 } : null, latest_cloud_audit: latestCloudAudit ? { snapshot_id: latestCloudAudit.snapshot_id || latestCloudAudit.audit_id, cloud_upload_verification: latestCloudAudit.cloud_upload_verification || latestCloudAudit.status, conclusion: latestCloudAudit.conclusion, next_action: latestCloudAudit.next_action, account: latestCloudAudit.account || null, manifest_file_id: latestCloudAudit.manifest_file_id || null, recovery_index_file_id: latestCloudAudit.recovery_index_file_id || null, mirror_manifest_generated_at: latestCloudAudit.mirror_manifest_generated_at || null, canonical_mirror_visible: latestCloudAudit.canonical_mirror_visible === true } : null, cleanup_allowed: cloudAuditMatchesMirror && latestCloudAudit.cleanup_allowed === true && mirrorManifest.cleanup_allowed === true },
