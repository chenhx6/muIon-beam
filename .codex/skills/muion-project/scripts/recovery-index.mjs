...
export function buildRecoveryIndex(root, { driveRoot, git = args => runGit(root, args).stdout.trim(), now = nowIso() } = {}) {
  ...
  const cloudAudits = listJson(snapshotDir).map(file => readJson(file)).filter(item => ['snapshot-cloud-audit', 'drive-cloud-visibility-audit'].includes(item?.record_type)).sort((a, b) => String(a?.recorded_at || a?.audited_at || '').localeCompare(String(b?.recorded_at || b?.audited_at || '')));
