...
for (const file of walkFiles(outbox, { ignoredDirectories: ['.git'] }).filter((item) => item.endsWith('.json'))) {
  ...
  if (state.snapshot_id) {
    const newer = walkFiles(stateDir, { ignoredDirectories: ['.git'] })
      ...
      .find((item) => item.state.drive_path && runGit(root, ['merge-base', '--is-ancestor', state.local_commit, item.state.local_commit], { allowFailure: true }).status === 0 && spawnSync(process.execPath, [path.join(import.meta.dirname, 'audit-project-snapshot.mjs'), '--project-root', root, '--drive-path', item.state.drive_path], { cwd: root, encoding: 'utf8', timeout: 180000 }).status === 0);
    if (newer) {
  ...
  if (!state.drive_path || !state.gitee_tag) {
    results.push({ file, status: 'invalid', error: 'missing Drive path or Gitee tag; refusing implicit defaults' });
    continue;
  }
  const commandArgs = state.snapshot_id
    ? [path.join(import.meta.dirname, 'sync-project-snapshot.mjs'), '--project-root', root, '--snapshot-id', state.snapshot_id, '--expected-commit', state.local_commit || 'missing', '--tag', state.gitee_tag, '--drive-path', state.drive_path]
    : [script, '--project-root', root, '--run-dir', runDir, '--run-id', state.run_id || path.basename(runDir), '--task-id', state.task_id || '', '--tag', state.gitee_tag, '--drive-path', state.drive_path];
...
process.exitCode = results.some((item) => ['invalid', 'still-pending'].includes(item.status)) ? 2 : 0;
