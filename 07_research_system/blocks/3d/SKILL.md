...
本 Block 与 `07_research_system/blocks/comsol/`、`07_research_system/blocks/geant4/` 同级，只负责三维几何。入口为 `node 07_research_system/blocks/3d/index.mjs --task path/to/geometry-task.json`，或 `--mode create|modify|inspect|export`。四个模式可以独立调用，不强制串行。
