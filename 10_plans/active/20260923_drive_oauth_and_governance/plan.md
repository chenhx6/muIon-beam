...
- connector profile 仍返回 `malakarlio28@gmail.com`，读取目标 folder 返回空列表。
...
1. **P3 OAuth 对齐**：在 connector 自己的连接设置中断开 28 账号，重新用 29 账号授权；不在对话中传递 token/API key。
2. **P4 API 读回**：读取 connector profile、canonical folder、manifest 和 recovery index；对照浏览器 folder ID 和 Gitee commit。
...
connector OAuth 仍绑定 `malakarlio28@gmail.com`。下一步是把 connector 连接本身切换到
`malakarlio29@gmail.com`，然后重新执行 P3/P4 读回验证。
