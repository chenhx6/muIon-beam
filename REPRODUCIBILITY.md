...
Drive 目标是本地设备损坏后直接接续重要工作。P4 整理采用 copy-only：先清点文件数量、大小和 SHA256，把内容写入 canonical 目录；新目录和 recovery index 通过校验后，可按用户已授权的清单删除冗余旧目录。ADR-007 另允许删除确认无用且无依赖的设计期原型生成物，并保留删除 receipt。`.git`、session runtime、缓存和未验证 outbox 不进入 Drive 项目快照。
