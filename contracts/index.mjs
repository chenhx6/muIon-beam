...
export function freezeContract(input, { root = process.cwd(), sourcePath = null } = {}) {
  ...
  const contract = checked.contract; const directory = path.join(root, 'contracts', 'instances', contract.contract_id); const file = path.join(directory, 'contract.yaml');
...
export function writeAttemptReport(frozen, report, { root = process.cwd(), attemptId = makeId('ATT') } = {}) {
  const directory = path.join(root, 'contracts', 'instances', frozen.contract.contract_id, 'attempts', attemptId); fs.mkdirSync(directory, { recursive: true });
