...
test('Drive mirror copies the local relative tree and verifies each file', t => {
  ...
  fs.mkdirSync(drive, { recursive: true }); fs.writeFileSync(path.join(drive, 'old-archive.txt'), 'retain for cleanup gate\n');
  ...
  assert.equal(first.manifest.file_count, 2); assert.equal(first.manifest.copied_count, 2); assert.equal(first.manifest.stale_count, 1); assert.deepEqual(first.manifest.stale_files, ['old-archive.txt']); assert.equal(first.manifest.cleanup_allowed, false);
